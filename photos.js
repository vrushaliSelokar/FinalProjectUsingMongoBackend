...
router.post("/", async function (req, res) {
  let dbo = await client.db("testdb");
  var myobj = req.body;
  let data = await dbo.collection("photos").insertOne(myobj);
  console.log(data);
...
router.get("/:id", async function (req, res) {  //to access only single data at a time
  console.log("i am id= ",req.params.id);
      ...
      let dbo = await client.db("testdb");
      let data = await dbo.collection("photos")
...
router.put("/:id",async function (req, res) {
  ...
  let dbo = await client.db("testdb");
      let data = await dbo.collection("photos").updateOne(myquery,newvalues);
...
router.delete("/:id",async function (req, res) {  //to access only single data at a time
  ...
  let dbo = await client.db("testdb");
      let data = await dbo.collection("photos").deleteOne(myquery);
